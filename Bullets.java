...
/**
 * Creates the class for the bullet. It manipulates where the bullet is, where
 * it is going, and what type it is.
*
 */
public class Bullets {
    ...
    /**
     * @constructor @param (bulletType): depending on the gun, will shoot the
     * specific bullet type
     * @param (x): x-axis position of the bullet
     * @param (y): y-axis position of the bullet
     * @param (slope): used to calculate the magnitude, as the slope increases
     * it should not make the bullet faster
...
*
    ...
    public void draw(Graphics2D g2d) {
        ...
        if (!inverseY && !inverseX) {
            rotation = -Math.acos(1 / magnitude); 
        }else if (inverseX && !inverseY) {
            rotation = Math.acos(1 / magnitude) + Math.toRadians(180); 
        }else if (inverseX && inverseY) {
            rotation = -Math.acos(1 / magnitude) + Math.toRadians(180); 
        }else {
            rotation = Math.acos(1 / magnitude);
